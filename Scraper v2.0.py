...
STABLECOINS = {'USDT', 'USDC', 'BUSD', 'DAI','BSC-USD','USD1','CBBTC', 'WETH','UST', 'TUSD', 'USDP', 'USDD', 'FRAX', 'GUSD', 'LUSD', 'FDUSD'}
...
def get_flipping_emoji(flipping_multiple):
    if flipping_multiple >= 10: return "🚀🚀🚀"
    elif flipping_multiple >= 5: return "🚀🚀"
    elif flipping_multiple >= 3: return "🚀"
    elif flipping_multiple >= 2: return "🔥"
    elif flipping_multiple >= 1.5: return "⭐"
    else: return "✅"
...
def create_html_report(hot_tokens, save_path):
    ...
    html_content += f"""
        <div class="footer">
            <p>Generated by Volume Tracker v2.0 | By (@traderabba)</p>
        </div>
    </body>
    </html>
    """
...
def fetch_coingecko():
    ...
    print(f"✅ {len(tokens)} tokens from CoinGecko")
    return tokens
...
def fetch_coinmarketcap():
    ...
    print(f"✅ {len(tokens)} tokens from CoinMarketCap")
    return tokens
...
def fetch_livecoinwatch():
    ...
    print(f"✅ {len(tokens)} tokens from LiveCoinWatch")
    return tokens
...
def fetch_coinrankings():
    ...
    print(f"✅ {len(tokens)} tokens from CoinRankings")
    return tokens
...
def fetch_and_save():
    print("   👁️ Be patient bro...")
    print("   📡 Scanning for High-volume in the Last 24 Hours..")
    print("   🎯 Other Tokens Volume > 75% of MC")
    print("   📢  Don't Second-guess Large-Cap Tokens ($1B+)")
    print("   ✅ Large-cap Volume >= 50% of their MC")
    print("=" * 50)
    ...
    print(f"📊 Raw results: CG={len(cg_data)}, CMC={len(cmc_data)}, LCW={len(lcw_data)}, CR={len(cr_data)}")
    ...
    print(f"✅ {len(hot_tokens)} high-volume tokens at {now}")
    print(f"💾 Saved CSV: {csv_file}")
    print(f"🌐 Saved HTML: {html_file}\n")
    
    if hot_tokens:
        print("🏆 HIGH-VOLUME TOKENS:")
        print("=" * 80)
        for i, token in enumerate(hot_tokens):
            emoji = get_flipping_emoji(token['flipping_multiple'])
            large_cap_indicator = " 💎" if token['large_cap'] else ""
        ...
        print(f"💎 Large-cap (>$1B): {large_cap_count} tokens")
        
    else:
        print("❌ No high-volume tokens found")

# Main Loop
print("  🚀 SPOT TRADING 24HR VOLUME HUNTER v2.0")
print("  😍 Brought to You by Trader Abba")
print("  👉 Follow me @TraderAbba ")
print("  ⏰ Auto-updates every hour")
print("  🎯 General Tokens Volume > 75% of MC")
print("  💎 Large-cap volume >= 50% of MC")
print("=" * 50)
